...
## K7. GalaxyField — a distant, authored star cluster during space-transition

The user proposed three more Sketchfab `.glb` downloads directly into the repo: `earth.glb`, `earth (1).glb` (both claimed as "definitely better, more colorful" replacements for the Earth itself), and `need_some_space.glb` (a candidate space/galaxy background). All three were inspected before any integration — none of this was taken on faith.

- **Inspection method**: GLB is a binary container (JSON scene graph + binary blob), not something you can eyeball from a file browser. Used `@gltf-transform/cli inspect` (temporary dev dependency, removed after use, same discipline as `sharp`/`playwright` elsewhere in this project) to read out the actual scene graph, materials, and texture resolutions without guessing, then a small script against `@gltf-transform/core` to extract embedded texture bytes to real image files for visual inspection.
- **Both `earth.glb` and `earth (1).glb` were rejected — verified, not assumed.** Extracted and viewed their `baseColorTexture` (8192×4096 each, identical between the two files): the continent silhouettes do not match real Earth at all — no recognizable Africa, Europe, or Americas outlines, despite correctly-placed white polar caps at top/bottom (i.e. a real equirectangular projection convention, just fictional content). Cropping the same Beijing coordinate window used throughout this project's texture QA showed empty ocean, confirming this isn't a different-but-real dataset with an unfamiliar look — it's a generic "Earth-like" stock planet asset, not satellite imagery. This directly conflicts with the project's standing "real geography only" requirement (real destination cities, real coordinates) regardless of resolution or color grading, so neither was integrated. Also moot on resolution alone: 8192×4096 is lower than the NASA-sourced 16000×8000 already in use (§K6). Left un-committed in the repo (too large, unused) pending the user's call on whether to delete them outright.
- **`need_some_space.glb` (renamed `public/assets/space/galaxy.glb`) held up and was integrated.** Not an image at all — a single `POINTS`-mode mesh, 50,000 vertices, per-vertex `COLOR_0` and no texture. Extracting and sampling the actual color attribute values (not just trusting the visual impression) showed genuine, physically-plausible variation: mostly cool blue-white "hot star" tones (`~0.75-0.9, 0.8-0.95, 1.0`) with occasional warm outliers (`~1.0, 0.9, 0.8`) — an authored palette, not uniform noise. Rendered standalone (a throwaway static HTML + raw three.js page, screenshotted via Playwright against a local static server — kept outside the actual app until the shape/color were confirmed worth using) it reads as a flattened, dense-cored star cluster, distinct in character from this project's own procedural/seeded star fields.
- **`three/GalaxyField.tsx`** — loads the GLB via drei's `useGLTF`, finds the `Points` mesh, and re-tunes its material for this project's aesthetic rather than trusting the source defaults: small `size` (0.28) with `sizeAttenuation`, `AdditiveBlending`, `depthWrite: false`, and critically a much lower peak opacity (0.22) than a naive "just show it" attempt — at opacity 1 the cluster blows out into a single flat white blob and loses all the color variation that made it worth using in the first place; only at a restrained opacity do the individual points and their color variation stay visible.
  - Positioned off-axis (`[40, 20, -80]`, scaled ×8) — far enough and to one side that it never competes with Earth or the marker labels for visual weight, matching `StarField`'s existing "supporting imagery, never the primary content" rule (§7).
  - **A real, silent bug caught only by testing**: an early version placed it at `z=-150`, well past `EarthCanvas`'s `<Canvas>` camera `far: 100` clipping plane — it rendered nothing, with no error of any kind, and even forcing the material to `opacity: 1` produced an empty frame. Only concluded it was a clipping issue (not a load failure, wiring bug, or opacity/timing issue) after confirming the GLB loaded successfully and methodically ruling out the other explanations. A reminder that "no console error" and "no page error" are not proof a WebGL object is actually rendering — the browser doesn't warn about content past the far plane, it simply never draws it.
  - Fade timing reuses the same `revealed` prop `EarthCanvas` already receives: fades in (opacity 0 → 0.22) during `space-transition`, then fades back to 0 once `storyPhase` reaches `'earth'` — a genuinely different envelope from the reveal timeline in §K3, not the same tween reused, since this element's whole job is to belong to the transition beat and get out of the way once Earth itself is the focus. `prefers-reduced-motion` collapses both directions to a flat 0.4s fade, matching every other reduced-motion path in this project.
  - Verified end-to-end in the actual running app (not just the standalone preview): visible and subtle in the upper-right during the space-transition text beats, fully faded out by the time Earth is legible on screen, no console/page errors.
  - **Not yet done, explicitly deferred by the user**: `GalaxyField`'s current placement (a subtle corner element near Earth) is not its intended final home. The user's actual request is for this same point cloud to become an *interactive background for the Intro/Question/Answer flow* — replacing or sitting behind the current `AmbientBackground` — with camera movement (rotating, flying into the point cloud, or orbiting to view it from the side) tied to question/answer progress. That is a separate, larger architectural change (introducing WebGL earlier than `space-transition`, which §6/§10 currently treat as a deliberate lazy-loading boundary — "no WebGL cost during the question flow" — so relaxing it has real bundle-size/performance implications to weigh, not just a repositioning) and has not been started.

## K8. Earth re-sourced from `earth.glb` — real geography intentionally dropped, by explicit user decision

The user provided two more Sketchfab downloads directly in the repo (`earth.glb`, `earth (1).glb`) insisting they were "definitely better, more colorful" than the NASA-sourced Earth from §K6, and asked outright whether GLB was even a format this session could open.

- **Confirmed capability first**: GLB was never a blocker — §K7 had already parsed and extracted textures from three separate `.glb` files via `@gltf-transform/cli`/`core` (temporary dev dependencies, removed after use). The rejection in §K7 was based on what was actually inside the file, not an inability to open it.
- **Re-verified the "not real geography" finding under direct challenge, with much stronger evidence than the first pass.** The user disputed §K7's conclusion, asserting the map inside `earth.glb` was real and pins could be placed on it the same way. Rather than argue from the single earlier Beijing data point, cropped four more of the most unmistakable silhouettes on the planet (UK/Ireland, Florida, India, Italy/the Mediterranean) from the exact same `baseColorTexture` at their real coordinates, using the same UV formula verified throughout this project. All four are simply open ocean in this texture; a landmass appears near Italy's coordinates but in roughly Morocco's position instead. This was shown to the user directly (five confirmed-absent real landmarks, not one) before asking again how to proceed.
- **User's decision, made with full knowledge of that evidence**: use `earth.glb` anyway. Real geography is no longer a goal for this planet — waypoint pins (Beijing/Tokyo/Cairo, unchanged lat/lng from `data/journey.ts`) are now purely decorative placement on whatever terrain happens to be there, not a claim about a real place. This is a deliberate reversal of a requirement stated repeatedly since early in this project ("real destinations, real coordinates") — recorded here as an explicit decision, not a silent regression, precisely so a future reader doesn't mistake it for one.
- **Implementation reused the existing shader pipeline rather than replacing it wholesale.** `earth.glb`'s own PBR material (`baseColorTexture` + `emissiveTexture` + `normalTexture` + `metallicRoughnessTexture`, all 8192×4096, generated by Sketchfab's own exporter) is designed for a generic real-time PBR viewer — dropped in as a standard `MeshStandardMaterial`, its emissive (night-lights) texture would glow on the daylit side too, since glTF's spec has no concept of a sun-relative terminator. That would have thrown away exactly the "real lighting, real day/night switching" quality the user specifically praised. Instead, all six textures were extracted from the GLB and re-encoded as drop-in replacements for `public/assets/earth/`'s existing files, keeping `earthShader.ts`'s custom terminator blending, Blinn-Phong specular, fine-grain procedural detail, and cinematic grade (§K5) fully intact — the *source imagery* changed, the *rendering* didn't:
  - `earth-day.webp` ← `baseColorTexture` (unchanged, re-encoded)
  - `earth-night.webp` ← `emissiveTexture`
  - `earth-normal.webp` ← `normalTexture`
  - `earth-specular.webp` ← `metallicRoughnessTexture`'s **green channel** (glTF packs roughness there; R and B are flat/unused on this asset), **inverted** — the shader's specular mask wants high values on glossy ocean and low on matte land, i.e. the opposite sense from roughness.
  - `earth-clouds.webp`/`-mobile` ← the separate clouds mesh's `baseColorTexture`, which is a 2-channel (grayscale + alpha) image where the **alpha channel**, not the grayscale channel, carries the real density data (verified by sampling per-channel means, not assumed) — extracted and written out as the plain opaque grayscale image `cloudShader.ts` expects.
- **A real texture-processing problem, found and only partially solvable.** The clouds' source alpha channel is heavily skewed toward high values almost everywhere (mean ≈184/255) — this specific asset's clouds are close to fully overcast by design, not a sparse "clear patches with occasional storms" map. A black-point-raise and several gamma curves (up to γ=4) were tried and measured (not eyeballed) — none meaningfully reduced the *effective* coverage, because a large fraction of pixels are already near-maximum and unaffected by any midtone curve. The practical fix was in `cloudShader.ts` itself rather than the texture: the alpha ceiling (`mix(0.22, 0.62, lit)` → `mix(0.14, 0.38, lit)`) was lowered so even dense source coverage reads as haze rather than a solid blanket. Flagged here as a known limitation of this specific source asset, not fully resolved — surface and ocean specular are visible through the clouds now, but the sky is still more overcast than the previous NASA-sourced cloud texture gave.
- **Deleted, not left dormant: the regional-detail LOD system (`RegionalDetail.tsx`, `data/regionalTextures.ts`, `public/assets/earth/regions/*.webp`, and the corresponding shader uniforms in `earthShader.ts`/`Earth.tsx`).** §K6 had already flagged it as redundant once the base map matched its own source's resolution; it's now also structurally meaningless (grafting a real NASA photo of the real Bohai Bay onto a planet with no real Bohai Bay makes no sense) and was no longer even mounted in `EarthCanvas.tsx`. Removed outright rather than left as unreferenced code, per the standing "delete what's unused" rule — a future real-geography pass would need to rebuild this from scratch against whatever the day map is at that time, not resurrect crops tied to an abandoned source.
- Verified in-browser at all three waypoints (Beijing/Tokyo/Cairo arrivals) after every change in this pass: day/night terminator, ocean specular glint, both cloud decks, and marker labels all render correctly against the new source; zero console/page errors.
